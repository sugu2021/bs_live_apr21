...
@Component({
  selector: 'app-portfolioitems',
  templateUrl: './portfolioitems.component.html',
  styleUrls: ['./portfolioitems.component.css']
})
export class PortfolioitemsComponent implements OnInit {
  ...
  ngOnInit() {
    let ids = this.myPortfolioItems.map(item => item.url);
    this.filteredData = this.myPortfolioItems.filter(x => x.url === this.route.snapshot.params['url']);
